...
grep -n '<' apps/portfolio/data.json | grep -v 'http'
...
ls -lh apps/portfolio/worker.js

# Identify large components
node -e "
const fs = require('fs');
const worker = fs.readFileSync('apps/portfolio/worker.js', 'utf8');
...
grep "sha256-" apps/portfolio/worker.js | head -5
...
grep -n '`' apps/portfolio/index.html
grep -n '\$' apps/portfolio/index.html

# The build script should escape these automatically
# If not, check apps/portfolio/lib/utils.js escapeTemplateLiterals function
```
...
ls -la apps/portfolio/downloads/*.pdf

# Verify data.json paths
grep "pdfUrl" apps/portfolio/data.json
...
```bash
# Add domain to CSP in apps/portfolio/lib/security-headers.js
...
```bash
# Find HTTP URLs
grep -r "http://" apps/portfolio/

# Replace with HTTPS
sed -i 's|http://|https://|g' apps/portfolio/data.json
```
