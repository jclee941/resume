...
전반적으로 **production-safe**. 가장 시급한 P0는 "**Cloudflare 자격증명 강화**" + "**Jest
threshold 정상화**".
...
2. 이번 주: P0-2 (KV cookie encrypt), P0-4 (JOB_SERVICE binding), P1-2/3 (auth/CSRF
   gaps) — 1일
3. 이번 sprint: P1 나머지 + 가장 큰 P2 (validator consolidation, BUILD.bazel cleanup,
   CHANGELOG fix) — 3-5일
...
tracked 안 됨. 디스크에는 평문이지만 git history에는 없음. P0 → 디스크 수준 secret manager 권고로 강등.
