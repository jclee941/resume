...
- **Wave 2 — infrastructure EXECUTED, live validation still gated** (commit `63b72a38`, CI green): `BrowserSessionDO` rewritten as a real session broker over `@cloudflare/puppeteer` `sessions()`/`limits()`/`acquire()` (pure logic in `browser-session-broker.js`, 24 offline tests), plus a `withBrowserSession()` drop-in. **Additive/unwired** — nothing calls it, so live automation is untouched. Still unverified against **live Browser Rendering**: cross-worker `connect(sessionId)` reuse, `keep_alive` window, real concurrency accounting. That validation gates wiring it into a crawler.
