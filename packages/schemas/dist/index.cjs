...
__export(index_exports, {
  ...
  foreignAtsApplicationPacketMetadataSchema: () => foreignAtsApplicationPacketMetadataSchema,
  foreignAtsApplicationPacketSchema: () => foreignAtsApplicationPacketSchema,
  foreignAtsLocationSchema: () => foreignAtsLocationSchema,
  foreignAtsPlatformSchema: () => foreignAtsPlatformSchema,
  foreignAtsSourceSchema: () => foreignAtsSourceSchema,
  idSchema: () => idSchema,
...
// src/application-dashboard.js
var import_zod2 = require("zod");
var applicationPriorityWideSchema = import_zod2.z.enum(["low", "medium", "high"]);

// src/application-core.js
var import_zod3 = require("zod");
var applicationStatusSchema = import_zod3.z.enum([
  "pending",
  "saved",
  "applied",
  "viewed",
...
var applicationStatusWideSchema = import_zod3.z.enum([
  "pending",
  "saved",
  "applied",
  "viewed",
...
var applicationCreateSchema = import_zod3.z.object({
  jobId: idSchema,
...
// src/foreign-ats-application.js
var import_zod4 = require("zod");
var foreignAtsPlatformSchema = import_zod4.z.enum([
  "ashby",
  "greenhouse",
  "lever",
  "smartrecruiters",
...
var foreignAtsLocationSchema = import_zod4.z.object({
  countryCode: import_zod4.z.string().regex(/^[A-Z]{2}$/, "must be ISO 3166-1 alpha-2 uppercase"),
  region: import_zod4.z.string().min(1).max(100).optional(),
  city: import_zod4.z.string().min(1).max(100).optional(),
  remote: import_zod4.z.boolean().optional()
}).strict();
var foreignAtsSourceSchema = import_zod4.z.object({
  platform: foreignAtsPlatformSchema,
  sourceUrl: import_zod4.z.string().url().max(2e3),
  externalJobId: import_zod4.z.string().min(1).max(200).optional(),
  requisitionId: import_zod4.z.string().min(1).max(200).optional(),
  location: foreignAtsLocationSchema.optional()
}).strict();
var foreignAtsApplicationPacketMetadataSchema = import_zod4.z.object({
...
// src/application.js
var dashboardJobBaseSchema = import_zod5.z.object({
  position: import_zod5.z.string().min(1).max(500).optional(),
  title: import_zod5.z.string().min(1).max(500).optional(),
  company: import_zod5.z.string().min(1).max(200).optional(),
  location: import_zod5.z.string().max(1e3).optional(),
  notes: import_zod5.z.string().max(1e3).optional(),
  source: import_zod5.z.string().max(1e3).optional(),
  platform: import_zod5.z.string().max(1e3).optional(),
  sourceUrl: import_zod5.z.string().url().max(2e3).optional(),
  source_url: import_zod5.z.string().url().max(2e3).optional(),
  jobUrl: import_zod5.z.string().url().max(2e3).optional(),
  job_url: import_zod5.z.string().url().max(2e3).optional(),
  priority: applicationPriorityWideSchema.optional(),
  status: applicationStatusWideSchema.optional(),
  matchScore: import_zod5.z.coerce.number().min(0).max(100).optional(),
  match_score: import_zod5.z.coerce.number().min(0).max(100).optional(),
  matchPercentage: import_zod5.z.coerce.number().min(0).max(100).optional(),
  match_percentage: import_zod5.z.coerce.number().min(0).max(100).optional()
});
var dashboardApplicationCreateSchema = import_zod5.z.object({
...
var dashboardApplicationUpdateSchema = import_zod5.z.object({
  notes: import_zod5.z.string().max(5e3).optional(),
  priority: applicationPriorityWideSchema.optional(),
  resumeId: import_zod5.z.string().max(100).optional()
}).passthrough();
var dashboardStatusUpdateSchema = import_zod5.z.object({
...
var periodStringSchema = import_zod6.z.string().regex(
  /^(\d{4}\.\d{2}|\d{4})\s*[~\-–]\s*(\d{4}\.\d{2}|\d{4}|현재|Present|現在)$/,
  'must be "YYYY.MM ~ YYYY.MM" or "YYYY.MM ~ \uD604\uC7AC/Present/\u73FE\u5728"'
);
var skillLevelSchema = import_zod6.z.enum(["beginner", "intermediate", "advanced", "expert"]);
var resumeSkillItemSchema = import_zod6.z.object({
  name: import_zod6.z.string().min(1),
  level: skillLevelSchema.optional(),
  proficiency: import_zod6.z.number().int().min(0).max(100).optional()
});
var resumeSkillCategorySchema = import_zod6.z.object({
  title: import_zod6.z.string().min(1),
  icon: import_zod6.z.string().optional(),
  items: import_zod6.z.array(resumeSkillItemSchema)
});
var resumeCareerSchema = import_zod6.z.object({
  company: import_zod6.z.string().min(1),
  period: periodStringSchema,
  role: import_zod6.z.string().optional(),
  description: import_zod6.z.string().optional()
});
var resumeProjectSchema = import_zod6.z.object({
  name: import_zod6.z.string().min(1),
  role: import_zod6.z.string().optional(),
...
var resumeCertificationSchema = import_zod6.z.object({
  name: import_zod6.z.string().min(1),
  issuer: import_zod6.z.string().min(1),
  date: import_zod6.z.string().min(1),
  expirationDate: import_zod6.z.string().nullable().optional()
});
var resumeEducationSchema = import_zod6.z.object({
  school: import_zod6.z.string().min(1),
  major: import_zod6.z.string().min(1),
  startDate: import_zod6.z.string().optional(),
  endDate: import_zod6.z.string().optional(),
  status: import_zod6.z.string().optional()
});
var resumePersonalSchema = import_zod6.z.object({
  name: import_zod6.z.string().min(1),
  email: emailSchema,
  phone: koreanPhoneSchema.optional(),
  github: import_zod6.z.string().url().optional(),
  portfolio: import_zod6.z.string().url().optional()
});
var resumeSchema = import_zod6.z.object({
  personal: resumePersonalSchema,
  careers: import_zod6.z.array(resumeCareerSchema),
  projects: import_zod6.z.array(resumeProjectSchema).optional(),
  skills: import_zod6.z.array(resumeSkillItemSchema).optional(),
  certifications: import_zod6.z.array(resumeCertificationSchema).optional(),
...
var adminLoginSchema = import_zod7.z.object({
  token: import_zod7.z.string().min(20)
});
var platformSessionSchema = import_zod7.z.object({
  platform: platformSchema,
  cookies: import_zod7.z.string().min(1),
  token: import_zod7.z.string().optional(),
  email: import_zod7.z.string().email().optional(),
  expiresAt: import_zod7.z.number().int().positive(),
  refreshedAt: import_zod7.z.number().int().positive().optional()
});
var sessionStateSchema = import_zod7.z.object({
...
var webhookSignatureHeaderSchema = import_zod8.z.object({
  "x-webhook-signature": import_zod8.z.string().regex(/^[a-f0-9]{64}$/, "must be 64-char hex HMAC-SHA256"),
  "x-webhook-timestamp": isoTimestampSchema.optional()
});
var notificationPayloadSchema = import_zod8.z.object({
...
var applicationStatusChangePayloadSchema = import_zod8.z.object({
  applicationId: import_zod8.z.string().min(1),
  oldStatus: import_zod8.z.string().min(1),
  newStatus: import_zod8.z.string().min(1)
});

// src/portfolio.js
var import_zod9 = require("zod");
var skillItemSchema = import_zod9.z.union([
  import_zod9.z.string().min(1),
  import_zod9.z.object({ name: import_zod9.z.string().min(1), level: import_zod9.z.string().optional() })
]);
var skillCategorySchema = import_zod9.z.union([
  import_zod9.z.array(skillItemSchema),
  import_zod9.z.object({ items: import_zod9.z.array(skillItemSchema) })
]);
var resumeItemSchema = import_zod9.z.object({
  title: import_zod9.z.string().min(1),
  description: import_zod9.z.string().min(1),
  stats: import_zod9.z.array(import_zod9.z.string()),
  highlight: import_zod9.z.boolean().optional(),
  completePdfUrl: import_zod9.z.string().optional(),
...
var certificationItemSchema = import_zod9.z.object({
  ...
  issuer: import_zod9.z.string().min(1),
  date: import_zod9.z.string().nullable().optional(),
...
var resumeDownloadSchema = import_zod9.z.object({
  pdfUrl: import_zod9.z.string().min(1),
  docxUrl: import_zod9.z.string().min(1),
  mdUrl: import_zod9.z.string().min(1)
});
var portfolioDataSchema = import_zod9.z.object({
...
0 && (module.exports = {
  ...
  emailSchema,
  foreignAtsApplicationPacketMetadataSchema,
  foreignAtsApplicationPacketSchema,
  foreignAtsLocationSchema,
  foreignAtsPlatformSchema,
  foreignAtsSourceSchema,
  idSchema,
  isoTimestampSchema,
