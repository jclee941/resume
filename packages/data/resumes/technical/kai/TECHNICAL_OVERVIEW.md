...
**문제**: 대규모 방화벽 룰 운영, 중복 및 미사용 룰 정리 필요
...
- 월간 위협 탐지
- False Positive 탐지 기준 정비 (시그니처 튜닝)
- 빠른 탐지 시간
...
2. **서버 운영**: 다수 Linux 서버 패치 관리 및 모니터링
3. **방화벽 정책 최적화**: 로그 분석을 통한 방화벽 중복 룰 정리
...
제조 환경은 일반 IT 환경과 다릅니다. 생산 시스템의 가용성이 최우선이며, 보안 패치도 생산 일정을 고려해야 합니다. 망분리를 통한 물리적
격리가 가장 확실한 방어책입니다.

### 2. 방화벽 정책 최적화의 중요성

대규모 방화벽 룰 중 상당수가 사용하지 않는 룰이었습니다. 로그 분석을 통해 불필요한 룰을 제거하여 정책 구조를 정리하고 관리 절차를 표준화하고 관리 복잡도를
줄였습니다.

### 3. 취약점 관리의 체계화

월간 취약점 스캔과 SLA 기반 패치 관리로 Critical 취약점을 SLA 기준 내에 처리했습니다. 정해진 프로세스가 보안 수준을 안정시킵니다.

### 4. 첫 직장에서 배운 기본기

이 프로젝트는 제 커리어의 시작이었습니다. Linux 서버 운영, 방화벽 정책 관리, 로그 분석 등 인프라·보안 엔지니어로서의 기본기를 탄탄히
다진 프로젝트입니다.
