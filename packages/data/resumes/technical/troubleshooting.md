...
- name: Generate Worker
  run: cd apps/portfolio && node generate-worker.js
```
...
html-minifier --collapse-whitespace \
  ...
  --minify-css true \
  --minify-js true \
  apps/portfolio/index.html -o apps/portfolio/index.min.html
```
...
node apps/portfolio/generate-worker.js
...
grep -n '`' apps/portfolio/worker.js | grep -v '\\'
```
...
**Current Implementation**: Working via `apps/portfolio/index.html` line 1100+ ✅
...
nodemon --watch apps/portfolio/index.html --watch apps/portfolio/resume.html \
  --exec "node apps/portfolio/generate-worker.js && wrangler dev"
```
...
```bash
node -c apps/portfolio/worker.js
```

### Check HTML size

```bash
wc -c apps/portfolio/index.html apps/portfolio/resume.html
```

### Verify UTF-8 encoding

```bash
file -bi apps/portfolio/index.html
