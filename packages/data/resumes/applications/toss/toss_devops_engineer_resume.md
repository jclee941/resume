...
9년간 금융·공공 분야에서 인프라 설계와 운영을 담당하며, 보안 인프라 구축, Observability 운영, 운영 자동화를 다뤄왔습니다.

특히 넥스트레이드 대체거래소에서 금융 시스템의 고가용성 인프라를 설계하고, Splunk·Prometheus·Grafana 기반의 모니터링 체계를
구축하면서 **"장애를 빠르게 감지하고, 개발자가 직관적으로 원인을 파악할 수 있는 시스템"** 의 중요성을 깊이 체감했습니다.

토스증권의 DevOps&SRE팀이 추구하는 **개발자 경험 개선**과 **Cloud Native 기반 대규모 시스템 운영** 방향성에 공감하며,
금융 도메인에서 쌓은 인프라 운영 경험과 자동화 역량을 바탕으로 기여하고자 지원합니다.

---

## 핵심 역량

- **Observability 체계 구축**: Prometheus + Grafana + Loki + ELK 기반 메트릭·로그 통합 모니터링
  플랫폼 설계 및 운영 (다수 메트릭 엔드포인트와 대시보드)
- **컨테이너 오케스트레이션**: EKS 기반 Kubernetes 클러스터 구축, Helm Chart 배포 자동화, Docker 기반 서비스
  운영 (홈랩 8 VM/CT)
...
**프로젝트 규모**: 대규모 서버·네트워크 장비 신규 구축 / 금융위 본인가 심사 통과
...
기술: AWS (EC2, VPC, IAM, S3, CloudTrail, GuardDuty), Terraform, Prometheus,
Grafana
...
2020.08 ~ 2021.08 (1년 9개월) | 대규모 컨택센터 인프라

- **대규모 인프라 자동화**: Ansible + Python 연동으로 대규모 서버 설정 동기화 자동화
...
- **Prometheus**: 다수 메트릭 엔드포인트 수집, Blackbox Exporter 기반 외부 서비스 헬스체크
- **Grafana**: 다수 대시보드 운영 (시스템, 컨테이너, 네트워크, 애플리케이션)
...
Proxmox VE 기반 가상화 플랫폼에서 여러 서비스 운영
...
AWS (EC2, VPC, EKS, IAM, S3, CloudTrail, GuardDuty), Cloudflare Workers, Proxmox
VE
