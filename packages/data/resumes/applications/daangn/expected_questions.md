...
## 1. 아이티센CTS (2025.03~2026.02) — Nextrade SOC 교대 운영 + Splunk SIEM + Claude AI

### 핵심 검증 질문

- "SOC 교대 운영를 운영하셨다고 했는데, 본인이 야간 당직일 때 P1 인시던트 떴습니다. 초기 대응 동안 정확히 무엇을 하시나요?"
- "Splunk 탐지룰을 고도화했다고 하셨는데, 가장 최근에 직접 만들거나 수정한 대표 룰를 화이트보드에 SPL로 써주실 수 있나요?"
- "탐지룰을 만들 때 정밀도(precision)와 재현율(recall) 중 어떤 것을 우선하시나요? 그 기준이 룰마다 다른가요?"
- "Claude AI로 위협정보를 자동화했다고 하셨는데, AI 응답을 그대로 신뢰하지 못하는 상황은 어떻게 처리하셨나요?
  hallucination 방어는요?"
...
- "다수 탐지룰 중 가장 많이 트리거된 룰은 무엇이었고, 그게 진짜 위협이었나요 아니면 오탐이었나요?"
- "MITRE ATT&CK과 매핑한 룰의 비율은 얼마나 됩니까? 매핑하지 못한 룰은 왜 그랬나요?"
- "Claude AI 자동화 도입 전과 후, 분석가가 처리하는 알림이 처리하는 알림 수가 어떻게 바뀌었나요?"

### 함정 질문 (의사결정 재검토)

- "지금 다시 Nextrade SOC를 처음부터 설계한다면, 가장 먼저 바꾸고 싶은 것은 무엇인가요?"
- "당근은 금융권보다 규제가 덜합니다. 상시 SOC 교대 같은 비용이 큰 체계가 정말 필요할까요? 다른 접근법이 있을까요?"

### 답변 준비 키워드

PICERL, Triage P0~P3, Sigma 룰, MITRE T1078/T1059.001, Splunk SPL, alert
correlation, Claude prompt engineering, JSON schema validation,
human-in-the-loop
...
Zero Trust, segmentation, micro-segmentation, AWS VPC + Security Group으로 매핑, IaC
drift detection, change management, 정책 dry-run, blue-green policy rollout
...
- "AWS 보안에서 가장 잘못 설계되어 있는 대표 패턴를 말한다면 무엇인가요?"
...
IAM Access Analyzer, CloudTrail S3 + Object Lock, VPC Flow Logs → Athena 쿼리,
GuardDuty findings → SNS → Slack, Service Control Policy(SCP), AWS Config rules,
IMDSv2
...
RBAC, Pod Security Standards (Restricted/Baseline/Privileged), NetworkPolicy,
OPA Gatekeeper, Falco, kube-bench, image scanning (Trivy), runtime security,
secrets in etcd
...
- "NSX-T로 micro-segmentation을 구축하셨다면, 정책을 어떤 단위로 나누셨나요? VM 기준? 애플리케이션 기준? 태그
  기준?"
...
- "APT 솔루션의 ML 모델 결과를 전적으로 신뢰할 수 있나요? 신뢰할 수 없다면 어떻게 보완하셨나요?"

### 함정 질문

- "조인트리에서 다룬 NSX-T, NAC, DLP, APT가 당근의 클라우드 네이티브 환경에서 그대로 쓸 수 있나요? 어떻게 변환되어야
  하나요?"

### 답변 준비 키워드

distributed firewall, security tag, EDR vs APT 차이, UEBA, sandbox 분석, DLP
context-aware policy, false positive tuning loop
...
4. **MITRE ATT&CK 대표 TTP** (T1078 Valid Accounts, T1059 Command Execution, T1538
   Cloud Service Dashboard)
5. **당근 대표 블로그 인용 인용 문장** (FDS 또는 IDP/DevSecOps)
...
### A. Proxmox 홈랩 (pve3, 복수 LXC와 VM)

| #   | 질문                                                                            | 답변 방향                                                                       |
| --- | ------------------------------------------------------------------------------- | ------------------------------------------------------------------------------- |
| P1  | "Proxmox 단일 노드인데 SPOF 아닌가요? 어떻게 대응하시나요?"                     | 솔직 인정 + Synology NAS 백업(vzdump zstd) + RTO 기준 + 향후 클러스터 확장 계획 |
...
| B3  | "대규모 IP인데 Cloudflare WAF 한도가 500K입니다. 왜 더 안 늘리시나요?"                      | confidence_level 점수화로 노이즈 제거 + GeoIP 컨텍스트로 우선순위 + 운영 가시성 확보 우선       |
...
| F2  | "수동 조회 대비 어느 정도 시간을 줄였다고 판단했나요?"      | 정책 조회 평균 작업 시간 측정 + 반복 횟수 기반으로 비교                      |
...
| #   | 질문                                                                                    | 답변 방향                                                                                                                  |
| --- | --------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| E1  | "회사 일도 바쁘신데 왜 이렇게 많은 개인 프로젝트를 운영하시나요?"                       | 회사에선 못 쓰는 도구 학습(Cloudflare/automation/Claude AI) + 실제 운영하면서 배운 것을 회사에 역수입 (Splunk-FortiNet 통합 패턴) |
...
| E3  | "당근 보안팀 합류하면 개인 프로젝트는 어떻게 하시나요?"                                 | 보안 침해 위험 없는 범위에서만 운영 + 회사 업무에 온전히 집중 + 개인 프로젝트는 학습 도구로 유지                           |
