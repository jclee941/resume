...
export const dashboardCreateCompatSchema = z.object({}).passthrough().superRefine((body, ctx) => {
  ...
  for (const field of STRING_FIELDS) {
    ...
    if (value !== undefined && value !== null) {
      ...
      else if (value.length > 1000) addIssue(ctx, `${field} must be 1000 characters or less`, [field]);
    }
  }
  ...
  if (status !== undefined && !VALID_APPLICATION_STATUSES_WIDE.includes(status)) {
    addIssue(ctx, `status must be one of: ${VALID_APPLICATION_STATUSES_WIDE.join(', ')}`, ['status']);
  }
...
export const dashboardUpdateCompatSchema = z.object({}).passthrough().superRefine((body, ctx) => {
  if (body.notes !== undefined) {
    if (typeof body.notes !== 'string') addIssue(ctx, 'notes must be a string', ['notes']);
    else if (body.notes.length > 5000) addIssue(ctx, 'notes must be 5000 characters or less', ['notes']);
  }
  if (body.priority !== undefined && !PRIORITIES.includes(body.priority)) {
    addIssue(ctx, `priority must be one of: ${PRIORITIES.join(', ')}`, ['priority']);
  }
  if (body.resumeId !== undefined && body.resumeId !== null) {
    if (typeof body.resumeId !== 'string') addIssue(ctx, 'resumeId must be a string', ['resumeId']);
    else if (body.resumeId.length > 100) addIssue(ctx, 'resumeId must be 100 characters or less', ['resumeId']);
  }
});

export const dashboardStatusCompatSchema = z.object({}).passthrough().superRefine((body, ctx) => {
  if (!body.status) {
...
function parseDashboardSchema(body, schema) {
  if (!body || typeof body !== 'object') return { valid: false, errors: ['Request body must be an object'] };
