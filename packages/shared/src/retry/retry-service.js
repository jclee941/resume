...
export class RetryService extends EventEmitter {
    ...
    while (attempt <= retryConfig.maxRetries) {
      ...
      try {
        ...
        await this.#locked(serviceName, () =>
          this.#success(serviceName, circuitConfig, gate, latencyMs)
        );
        ...
        await this.#locked(serviceName, () =>
          this.#failure(serviceName, circuitConfig, gate, latencyMs, error)
        );
  ...
  getStats() {
    const services = {};
    for (const [serviceName, value] of this.#stats.services.entries())
      services[serviceName] = format(value);
    return {
  ...
  #enter(name, config) {
    const c = getCircuit(this.#circuits, name);
  ...
  #reject(name, gate) {
    const s = serviceStats(this.#stats, name);
    s.circuitRejections += 1;
    this.#stats.totalCircuitRejections += 1;
    const error = Object.assign(
      new Error(`${name} circuit is open. Retry after cooldown period.`),
      {
        code: 'CIRCUIT_OPEN',
        key: name,
        platform: name,
  ...
  #success(name, config, gate, ms) {
    ...
    if (c.halfOpenSuccesses >= config.halfOpenMaxCalls) {
      close(c);
      this.emit('circuit:closed', { serviceName: name, reason: 'recovered', state: c.state });
    }
  }
  ...
  #failure(name, config, gate, ms, error) {
    ...
    if (gate.fromHalfOpen) {
      c.halfOpenActiveCalls = Math.max(0, c.halfOpenActiveCalls - 1);
      open(c, config, this.#clock);
      this.#emitOpen(name, 'half_open_failure', c);
      return;
    }
    c.failureCount += 1;
    if (c.failureCount >= config.failureThreshold) {
      open(c, config, this.#clock);
      this.#emitOpen(name, 'failure_threshold_reached', c);
    }
  }
