...
export async function fillCareers(careers) {
  ...
  for (const career of careers.slice(0, 5)) {
    ...
    const companyInputs = await this.page.$$('input[name*="company"], input[placeholder*="회사"], input[placeholder*="기업명"]');
    const lastCompany = companyInputs[companyInputs.length - 1];
    if (lastCompany) {
      await lastCompany.fill(career.company);
    }

    const positionInputs = await this.page.$$('input[name*="position"], input[placeholder*="직책"], input[placeholder*="직급"]');
...
export async function fillEducation(education) {
  const schoolInput = await this.page.$('input[name*="school"], input[placeholder*="학교"], input[placeholder*="학교명"]');
  if (schoolInput) {
    await schoolInput.fill(education.school);
  }

  const majorInput = await this.page.$('input[name*="major"], input[placeholder*="전공"], input[placeholder*="전공명"]');
  if (majorInput) {
    await majorInput.fill(education.major);
  }

  if (education.status) {
    const statusSelect = await this.page.$('select[name*="status"], select[name*="graduation"], select[name*="graduate"]');
    if (statusSelect) {
      await statusSelect.selectOption({ label: education.status });
    } else {
      const statusRadio = await this.page.$(`input[type="radio"][value="${education.status}"], input[type="radio"][name*="status"][value*="${education.status}"]`);
      if (statusRadio) {
...
export async function fillCertifications(certifications) {
  for (const cert of certifications.slice(0, 6)) {
    ...
    const certInputs = await this.page.$$('input[name*="cert"], input[placeholder*="자격증"], input[placeholder*="자격증명"]');
    ...
    if (cert.issuer) {
      const issuerInputs = await this.page.$$('input[name*="issuer"], input[placeholder*="발급기관"], input[placeholder*="기관"]');
