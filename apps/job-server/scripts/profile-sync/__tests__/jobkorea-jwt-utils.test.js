...
describe('jwt-utils', () => {
  ...
  it('returns true for an expired JWT', () => {
    const expiredPayload = Buffer.from(JSON.stringify({ exp: Math.floor(Date.now() / 1000) - 100 })).toString('base64url');
  ...
  it('returns true when JWT is within 60-second buffer of expiry', () => {
    const nearExpiryPayload = Buffer.from(JSON.stringify({ exp: Math.floor(Date.now() / 1000) + 30 })).toString('base64url');
