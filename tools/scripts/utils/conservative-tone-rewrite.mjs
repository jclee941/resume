...
const workflowAutomationProject = data.personalProjects?.find((p) => p.name === 'Automation');
if (workflowAutomationProject?.description?.includes('무인 운영 가능한')) {
  workflowAutomationProject.description = workflowAutomationProject.description
    .replace('무인 운영 가능한', '자동화 운영')
    .replace('운영 오케스트레이션 기반을 확보', '워크플로우 기반 운영 체계를 구성');
  log('personalProjects[workflow-automation]: 무인 운영 / 오케스트레이션 기반 확보 정리');
}
