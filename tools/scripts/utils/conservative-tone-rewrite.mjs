...
const ach0OldPattern = /^넥스트레이드 ATS 보안 인프라를 고가용성\(HA\) 기준으로 설계하고 금융위 본인가 심사를 통과시켰습니다\.$/;
...
if (data.summary.coreCompetencies?.[0]?.includes('금융위 본인가')) {
  data.summary.coreCompetencies[0] = '금융권 보안 인프라 구축·운영 (FortiGate HA, 본인가 심사 대응)';
...
if (JSON.stringify(oldExp) !== JSON.stringify(newExp)) {
  ...
  log('summary.expertise: "Security Infrastructure" → "Network Security", "IaC/Automation" → "Infrastructure as Code"');
}
...
if (data.achievements[3]?.includes('99.9% 이상 가용률 유지')) {
  data.achievements[3] = 'Grafana 12개 대시보드와 Prometheus 50+ 메트릭 엔드포인트를 운영하며 인프라 상태를 단일 뷰에서 분석하는 환경을 구성했습니다.';
...
if (fnp?.description) {
  ...
  d2 = d2.replace('보안운영 자동화의 기반 도구로 재사용하고 있습니다', '보안운영 자동화에 재사용하고 있습니다');
  if (d2 !== fnp.description) { fnp.description = d2; log('personalProjects[FortiNet]: 직접 개발/기반 도구 정리'); }
}
...
if (c2?.description?.includes('IaC 정착')) {
  c2.description = c2.description
    ...
    .replace('Terraform IaC로 VPC/SG 코드 관리 정착, 변경 추적성 확보', 'Terraform IaC로 VPC/SG 코드 관리, 변경 추적')
    .replace('CloudTrail + GuardDuty 통합 로그 분석 체계 수립', 'CloudTrail + GuardDuty 통합 로그 분석 구성')
