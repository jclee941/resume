...
if (ach3Old && ach3Old.includes('높은 가용성% 가용률')) {
  data.achievements[3] = 'Grafana 12개 대시보드, Prometheus 50+ 메트릭 엔드포인트 운영, 99.9% 이상 가용률 유지';
...
if (ach1Old?.includes('다수 탐지 룰')) {
  data.achievements[1] = 'Splunk ES 32개 탐지 룰 + FortiNet API 연동으로 보안 이벤트 실시간 알림 파이프라인을 구축·운영했습니다.';
...
if (ach2Old?.includes('대폭 절감')) {
  data.achievements[2] = '보안 이벤트 자동 대응 체계를 구축해 알림 노이즈를 줄이고, FortiManager API 기반 방화벽 정책 조회를 수동(콘솔 접속)에서 자동(스크립트 단축)으로 전환했습니다.';
...
if (ach5Old?.includes('침해사고 예방률 향상')) {
  data.achievements[5] = '조인트리 NSX-T 마이크로세그멘테이션 적용으로 동서(East-West) 트래픽 가시성을 확보하고, 분산 방화벽(DFW) 정책 중앙 관리 체계를 수립했습니다.';
...
if (ach7Old?.includes('감소 줄이고')) {
  data.achievements[7] = '메타넷 재택근무 환경에서 백신-VPN 프로파일 충돌을 분석·해결하고, 신규 사이트 3개소 네트워크를 설계·구축했습니다.';
...
if (ach8Old?.includes('다수 제거')) {
  data.achievements[8] = 'KAI 폐쇄망 환경에서 방화벽 중복 룰을 정리하고 21개월 연속 내부정보 유출사고 0건을 유지했습니다.';
...
const newStatement = '금융위원회 본인가 심사를 직접 통과시킨 securities trading platform 보안 아키텍처 설계자입니다. 9년차 DevSecOps/SRE로서 FortiGate HA · Splunk ES SIEM(32개 탐지 룰) · Terraform IaC · Ansible/Python 자동화 · Prometheus/Grafana 관측성 플랫폼을 직접 설계·운영해왔습니다. 금융·공공 환경에서 보안 인프라 설계부터 운영 자동화까지 풀 사이클을 책임진 경험이 강점입니다.';
...
const newHeadline = 'DevSecOps/SRE | 금융위 본인가 통과 보안 아키텍처 설계 | FortiGate HA · Splunk · Terraform · AWS | 9년';
if (oldHeadline !== newHeadline) {
  data.platformVariants.wanted.headline = newHeadline;
  log(`platformVariants.wanted.headline: rewrote (${newHeadline.length} chars, was ${oldHeadline?.length})`);
}
...
if (data.platformVariants?.jobkorea && oldJkHeadline !== newHeadline) {
  data.platformVariants.jobkorea.headline = 'DevSecOps/SRE - 금융위 본인가 통과 보안 아키텍처 설계 9년';
...
if (Array.isArray(data.hope?.locations) && data.hope.locations.length === 1 && data.hope.locations[0] === '서울') {
...
if (data.careers[2]?.description === 'AWS 보안 운영 (IAM/VPC/WAF), Terraform IaC 관리, CloudTrail/GuardDuty 로그 통합, Prometheus/Grafana 모니터링') {
...
if (data.careers[5]?.description === 'VPN/NAC 운영, 네트워크 스위치 관리, FortiGate 정책 운영, 재택근무 인프라 구축') {
...
if (data.careers[6]?.description === 'Linux 서버 운영, 방화벽 정책 관리, 자산/패치 관리, OA 시스템 운영') {
