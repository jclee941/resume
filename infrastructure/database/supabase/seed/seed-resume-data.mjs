...
function trackSeed(seed, context) {
  if (usedSeeds.has(seed)) {
    ...
    throw new Error(
      `Duplicate content key detected: "${seed}" used by both ${prev} and ${
        context
...
function buildCertificationStatements(certifications) {
  const results = ensureArray(certifications).map((certification, index) => {
    if (!certification.name)
      throw new Error(
        `certification at index ${index} missing "name" — content-based ID requires a stable key`
      );
...
function buildSkillStatements(categoryRows) {
  ...
  for (const categoryRow of categoryRows) {
    categoryRow.items.forEach((item, itemIndex) => {
      if (!item.name)
        throw new Error(
          `skill item at index ${itemIndex} in category "${
            categoryRow.key
...
function buildPersonalProjectStatements(projects) {
  const results = ensureArray(projects).map((project, index) => {
    if (!project.name)
      throw new Error(
        `personal project at index ${index} missing "name" — content-based ID requires a stable key`
      );
...
function generateSql(data) {
  ...
  const skillsWhere = `category_id IN (SELECT id FROM resume_skill_categories WHERE resume_id = ${escapeSql(
    PROFILE_ID
  )})`;
