...
describe('AutoApplier strategy methods', () => {
  describe('Strategy Exports', () => {
    test('wanted-strategy.js exists and exports applyToWanted', () => {
      const strategyPath = path.join(
        __dirname,
        '../../../apps/job-server/src/auto-apply/strategies/wanted-strategy.js'
      );
    ...
    test('jobkorea-strategy.js exists and exports applyToJobKorea', () => {
      const strategyPath = path.join(
        __dirname,
        '../../../apps/job-server/src/auto-apply/strategies/jobkorea-strategy.js'
      );
      expect(fs.existsSync(strategyPath)).toBe(true);
      const content = fs.readFileSync(strategyPath, 'utf8');
    ...
    test('saramin-strategy.js exists and exports applyToSaramin', () => {
      const strategyPath = path.join(
        __dirname,
        '../../../apps/job-server/src/auto-apply/strategies/saramin-strategy.js'
      );
      expect(fs.existsSync(strategyPath)).toBe(true);
      const content = fs.readFileSync(strategyPath, 'utf8');
  ...
  describe('Platform Labels', () => {
    test('all strategies use correct platform labels', () => {
      ...
      const jobkoreaContent = fs.readFileSync(
        path.join(strategiesDir, 'jobkorea-strategy.js'),
        'utf8'
      );
      ...
      const saraminContent = fs.readFileSync(
        path.join(strategiesDir, 'saramin-strategy.js'),
        'utf8'
      );
      ...
      const linkedinContent = fs.readFileSync(
        path.join(strategiesDir, 'linkedin-strategy.js'),
        'utf8'
      );
  ...
  describe('Strategy Barrel Export', () => {
    test('strategies/index.js exports all strategies', () => {
      const indexPath = path.join(
        __dirname,
        '../../../apps/job-server/src/auto-apply/strategies/index.js'
      );
