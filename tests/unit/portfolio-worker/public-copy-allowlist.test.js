...
const { auditSource, diffJsonPointers, flattenJsonPointers, isAllowedPublicCopyPath,
  validateSourceMapBootstrap } = require('../../e2e/fixtures/public-copy-source-audit');
const { normalizePublicValue, occurrenceAddress } =
  require('../../e2e/fixtures/public-copy-ledger-extractor');
const { canonicalBaselineCommand, serializeBaseline, serializeCompactSorted, sha256,
  validateBaseline, validateBaselineReceipt } =
...
function baselineFixture() {
  const document = {
    ...
    occurrences: [{
      locale: 'ko', route: '/ko/', state: 'initial',
      viewport: { key: 'desktop-1280x900', width: 1280, height: 900, dpr: 1 },
      kind: 'dom-text', selector: '#main-content', attribute: null,
      accessiblePath: null, occurrenceIndex: 0, value: '본문',
    }],
  };
  const bytes = serializeBaseline(document);
  const command = canonicalBaselineCommand({ baseSha: document.baseSha, sourceUrl: document.sourceUrl,
    output: '.omo/evidence/portfolio-copy-cleanup/ledger-baseline.json' });
  return { document, bytes, receipt: JSON.parse(serializeCompactSorted({
    version: 1, mode: 'baseline', baseSha: document.baseSha,
    liveHealthSha: document.expectedHealthSha, ledgerSha256: sha256(bytes),
    routes: ['/', '/ko/', '/en/', '/ja/'], occurrenceCount: document.occurrences.length,
    capturedAt: document.capturedAt, command,
  })) };
}
...
function hasForcedServerIsolation(config) {
  return config.includes("process.env.PORTFOLIO_FORCE_NEW_SERVER === '1'")
...
describe('public-copy ledger harness', () => {
  test('normalizes public values and includes accessiblePath in the address', () => {
    const occurrence = {
      locale: 'ko', route: '/ko/', state: 'initial',
      viewport: { key: 'desktop-1280x900', width: 1280, height: 900, dpr: 1 },
      kind: 'accessible-tree', selector: '#projects', attribute: null,
      accessiblePath: '/0/region~1name', occurrenceIndex: 0,
    };
  ...
  test('serializes a canonical, validated baseline', () => {
    ...
    expect(() => validateBaseline({ ...document, expectedHealthSha: 'b'.repeat(40) }))
      .toThrow('health SHA');
  });
  ...
  test('rejects malformed or forged baseline receipts', () => {
    ...
    expect(() => validateBaselineReceipt({ ...receipt, extra: true }, bytes))
      .toThrow('receipt keys');
    ...
    expect(() => validateBaselineReceipt(receipt, bytes.replace('본문', '위조')))
      .toThrow('receipt ledger digest');
    expect(() => validateBaselineReceipt(receipt, '{"version":2}\n')).toThrow();
  });

  test('ledger URL owns baseline navigation and conflicts fail explicitly', () => {
    expect(playwrightBaseUrl({ PORTFOLIO_LEDGER_URL: 'http://127.0.0.1:9' }))
      .toBe('http://127.0.0.1:9');
    expect(() => playwrightBaseUrl({
...
describe('public-copy source audit allowlist', () => {
  test('flattens arrays to exact JSON pointers and compares scalar leaves', () => {
    ...
    expect(diffJsonPointers({ a: 1 }, { a: 2 })).toEqual([
  ...
  test('English language names are immutable', () => expect(isAllowedPublicCopyPath('/languages/0/name', 'en')).toBe(false));
  ...
  test('KO duplicate cover blocks require byte-equal native owners', () => {
    ...
    expect(auditSource(before, after, 'ko', { en: after, ja: after })
      .every(({ allowed }) => allowed)).toBe(true);
    expect(auditSource(before, after, 'ko', {
      en: { coverLetter: { en: { headline: 'different' } } },
      ja: { coverLetter: { ja: { headline: '異なる' } } },
    }).every(({ allowed }) => allowed)).toBe(false);
    expect(isAllowedPublicCopyPath('/coverLetter/en/headline', 'ko')).toBe(false);
  });

  test('source-map bootstrap is exact and fail-closed', () => {
    const sourceMap = JSON.parse(fs.readFileSync(path.join(ROOT, 'tests/e2e/fixtures/public-copy-source-map.json'), 'utf8'));
    expect(validateSourceMapBootstrap(sourceMap)).toBe(sourceMap);
    const ref = { sourceOwner: 'apps/portfolio/index.html', jsonPath: '$.title', sourceSpan: null,
      decisionId: 'shell.test', bindings: { title: { sourceOwner: 'x.json', jsonPath: '$.title', value: 'x' } },
      matchMode: 'whole', fragmentPath: null };
    const entry = { locale: 'ko', route: '/ko/', state: 'initial', viewport: { key: 'desktop-1280x900', width: 1280, height: 900, dpr: 1 }, kind: 'dom-text', selector: '#title', attribute: null, accessiblePath: null, occurrenceIndex: 0, beforeFragments: {}, afterFragments: {}, beforeSourceRefs: [ref], afterSourceRefs: [] };
    expect(validateSourceMapBootstrap({ version: 1, entries: [entry] })).toBeTruthy();
    for (const badRef of [{ ...ref, decisionId: 42 }, { ...ref, decisionId: '  ' }, { ...ref, matchMode: 'fragment', fragmentPath: '' },
      { ...ref, bindings: { title: { ...ref.bindings.title, jsonPath: '/title' } } },
      { ...ref, sourceOwner: '/tmp/source.json' }, { ...ref, sourceOwner: '../source.json' },
      { ...ref, jsonPath: "$.['" }, { ...ref, matchMode: 'fragment', fragmentPath: '/missing' },
      { ...ref, bindings: { title: { ...ref.bindings.title, sourceOwner: '/tmp/x.json' } } },
      { ...ref, bindings: { title: { ...ref.bindings.title, sourceOwner: '../x.json' } } },
      { ...ref, bindings: { title: { ...ref.bindings.title, jsonPath: "$.['" } } }, { ...ref, sourceOwner: 'C:/source.json' }, { ...ref, sourceOwner: 'a\0b.json' }, { ...ref, sourceOwner: 'a\nb.json' }, { ...ref, sourceOwner: 'a\rb.json' }]) {
      expect(() => validateSourceMapBootstrap({ version: 1, entries: [{ ...entry, beforeSourceRefs: [badRef] }] })).toThrow();
    }
    const fragments = { f: { startUtf16: 0, endUtf16: 1, template: 'x' } };
    for (const fragmentPath of ['/f/template', '/f/startUtf16']) expect(() => validateSourceMapBootstrap({ version: 1, entries: [{ ...entry, beforeFragments: fragments, beforeSourceRefs: [{ ...ref, matchMode: 'fragment', fragmentPath }] }] })).toThrow();
    for (const mutation of [{ locale: 'en' }, { state: 'future' }, { kind: 'metadata', selector: 'meta:title' }, { kind: 'jsonld', selector: 'jsonld:0:/~2' }, { kind: 'manifest', selector: 'manifest:/manifest.json:/name~' }, { kind: 'accessible-tree', accessiblePath: '/name~' }, { kind: 'dom-attribute' }]) expect(() => validateSourceMapBootstrap({ version: 1, entries: [{ ...entry, ...mutation }] })).toThrow();
    expect(() => validateSourceMapBootstrap({ version: 1, entries: [entry, entry] })).toThrow();
    for (const invalid of [{ version: 1, entries: [], extra: true }, { version: 2, entries: [] },
  ...
  baseDiffTest('the repository remains unchanged relative to the supplied base', () => {
    ...
    const afterDocuments = Object.fromEntries(files.map((file) => [
      file.endsWith('_en.json') ? 'en' : file.endsWith('_ja.json') ? 'ja' : 'ko',
      JSON.parse(fs.readFileSync(path.join(ROOT, file), 'utf8')),
    ]));
    for (const file of files) {
      const before = JSON.parse(require('child_process').execFileSync('git', [
        'show', `${baseSha}:${file}`,
      ...
      const rejected = auditSource(before, after, locale, afterDocuments)
...
describe('ledger prerequisites', () => {
  ...
  test('force-new-server disables Playwright server reuse explicitly', () => {
    ...
    expect(hasForcedServerIsolation(config.replace(
      /reuseExistingServer:\s*forceNewServer\s*\?\s*false\s*:\s*!process\.env\.CI/,
      'reuseExistingServer: !process.env.CI'
    ))).toBe(false);
    expect(hasForcedServerIsolation(config.replace(
      "process.env.PORTFOLIO_FORCE_NEW_SERVER === '1'",
      'false'
    ))).toBe(false);
  });
});
