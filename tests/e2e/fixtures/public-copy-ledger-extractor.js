...
async function extractDom(page) {
  return page.evaluate(() => {
    ...
    for (let node = walker.nextNode(); node; node = walker.nextNode()) {
      ...
      if (!element || element.closest('script,style,template,noscript') || !visible(element))
        continue;
      push(
        'dom-text',
        element,
    ...
    for (const element of document.querySelectorAll(
      '[title],[alt],[placeholder],[aria-label],[aria-description],[download]'
    )) {
      ...
      for (const attribute of [
        ...
        if (element.hasAttribute(attribute))
          push('dom-attribute', element, element.getAttribute(attribute), attribute);
      }
    }
  ...
  return raw
    .map((item) => {
      ...
      const base = {
        ...
        accessiblePath: item.accessiblePath,
      };
