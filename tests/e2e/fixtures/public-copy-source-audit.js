...
function validateFragmentMap(value, label) {
  if (!value || typeof value !== 'object' || Array.isArray(value))
    throw new Error(`Invalid public-copy source map: ${label}`);
  if (!isSortedUnique(Object.keys(value)))
    throw new Error(`Invalid public-copy source map: ${label} order`);
  for (const [key, fragment] of Object.entries(value)) {
    ...
    if (
      !Number.isInteger(fragment.startUtf16) ||
      !Number.isInteger(fragment.endUtf16) ||
      fragment.startUtf16 < 0 ||
      fragment.endUtf16 <= fragment.startUtf16 ||
      typeof fragment.template !== 'string'
    )
...
function validateSourceRef(ref, fragments) {
  ...
  if (ref.sourceSpan !== null) {
    ...
    if (
      !Number.isInteger(ref.sourceSpan.startByte) ||
      !Number.isInteger(ref.sourceSpan.endByte) ||
      ref.sourceSpan.startByte < 0 ||
      ref.sourceSpan.endByte <= ref.sourceSpan.startByte ||
      !/^[0-9a-f]{64}$/.test(ref.sourceSpan.sha256)
    )
      throw new Error('Invalid public-copy source map: source span values');
  }
  for (const [key, binding] of Object.entries(ref.bindings)) {
    if (!/^[A-Za-z][A-Za-z0-9]*$/.test(key))
      throw new Error('Invalid public-copy source map: binding key');
    exactKeys(binding, ['sourceOwner', 'jsonPath', 'value'], 'binding');
    if (
      !isRepoRelative(binding.sourceOwner) ||
      typeof binding.value !== 'string' ||
      typeof binding.jsonPath !== 'string' ||
      !JSON_PATH.test(binding.jsonPath)
    )
      throw new Error('Invalid public-copy source map: binding values');
  }
  if (!isSortedUnique(Object.keys(ref.bindings)))
    throw new Error('Invalid public-copy source map: binding order');
}

function validateSourceMapBootstrap(sourceMap) {
  exactKeys(sourceMap, ['version', 'entries'], 'root');
  if (sourceMap.version !== 1 || !Array.isArray(sourceMap.entries))
    throw new Error('Invalid public-copy source map: identity');
  const keys = [
    'locale',
    'route',
    'state',
  ...
  const addresses = sourceMap.entries.map((entry) =>
    JSON.stringify(keys.slice(0, 9).map((key) => entry[key]))
  );
  ...
  for (const entry of sourceMap.entries) {
    ...
    const dimensions =
      entry.viewport.key === 'desktop-1280x900'
        ? [1280, 900]
        : entry.viewport.key === 'mobile-375x812'
          ? [375, 812]
          : [];
    const routeLocale = { '/': 'ko', '/ko/': 'ko', '/en/': 'en', '/ja/': 'ja' }[entry.route];
    if (
      !['ko', 'en', 'ja'].includes(entry.locale) ||
      routeLocale !== entry.locale ||
      !isFrozenLedgerState(entry.state) ||
      !KINDS.has(entry.kind) ||
      !isLedgerSelector(entry.kind, entry.selector) ||
      dimensions[0] !== entry.viewport.width ||
      dimensions[1] !== entry.viewport.height ||
      entry.viewport.dpr !== 1 ||
      (entry.kind === 'dom-attribute') !==
        (typeof entry.attribute === 'string' && Boolean(entry.attribute)) ||
      (entry.kind === 'accessible-tree') !== isRfc6901Pointer(entry.accessiblePath) ||
      !Number.isInteger(entry.occurrenceIndex) ||
      entry.occurrenceIndex < 0 ||
      !Array.isArray(entry.beforeSourceRefs) ||
      !Array.isArray(entry.afterSourceRefs)
    ) {
    ...
    for (const refs of [entry.beforeSourceRefs, entry.afterSourceRefs]) {
      if (!isSortedUnique(refs.map(JSON.stringify)))
        throw new Error('Invalid public-copy source map: ref order');
    }
  }
  return sourceMap;
}
